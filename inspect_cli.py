...
用法：
    ...
    uv run python inspect_cli.py --input-dir sample_images --output output/ --batch-api
...
from inspector.domains import DEFAULT_DOMAIN, DOMAIN_PROFILES
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--domain", choices=sorted(DOMAIN_PROFILES), default=DEFAULT_DOMAIN,
        help="任務領域：pcb（PCB 瑕疵巡檢，預設）｜uav（無人機空拍巡邏）",
    )
    parser.add_argument(
    ...
    parser.add_argument("--conf", type=float, default=None, help="偵測信心閾值（預設依 --domain）")
    ...
    parser.add_argument(
        ...
        help="Experimental：改用 Gemini Batch API（僅 gemini；非即時，需輪詢等待）",
    )
    parser.add_argument("--html", action="store_true", help="額外產出 report.html（深色主題樣式）")
...
def main() -> None:
    ...
    from inspector.report import _image_verdict, render_html, render_json, render_report

    domain = DOMAIN_PROFILES[args.domain]
    ...
    print(f"\n偵測完成（domain={domain.name}）：{len(batch.results)} 張影像")
    for r in batch.results:
        counts = Counter(f.detection.class_name for f in r.findings.findings)
        summary = ", ".join(f"{k}x{v}" for k, v in sorted(counts.items())) or "無偵測"
    ...
    html_note = ""
    if args.html:
        html_path = render_html(batch, args.output)
        html_note = f"（HTML：{html_path}）"
    meter = batch.meter
    ...
    print(f"報告：{report_path}（JSON：{json_path}）{html_note}")
