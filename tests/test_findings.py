...
def test_label_layout_avoids_collision_and_stays_on_canvas():
    canvas = (180, 100)
    first = _place_label_rect((80, 8, 105, 32), (88, 22), canvas, [])
    second = _place_label_rect((102, 9, 128, 33), (88, 22), canvas, [first])

    assert _intersection_area(first, second) == 0
    for left, top, right, bottom in (first, second):
        assert 0 <= left < right <= canvas[0]
        assert 0 <= top < bottom <= canvas[1]


def test_label_layout_clamps_right_edge():
    rect = _place_label_rect((170, 40, 179, 55), (90, 20), (180, 100), [])
    assert rect[2] == 180
    assert rect[0] == 90
