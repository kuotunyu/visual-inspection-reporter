...
> 偵測結果可追溯的巡檢報告產生器：本地 YOLO26 ONNX 偵測 + 商用 VLM API → 繁體中文巡檢報告（PCB／UAV 皆可透過 `--domain` 切換）
...
以本 repo 實測 usage 換算（token 數為 API 回傳值，單價為官方付費層定價；免費層實際帳單 $0）。**以下是特定日期查證的估算值，不是即時報價承諾**——換算單價會隨供應商定價調整而過時，重跑前請自行核對當前官方定價頁：
...
換領域只需要在 `src/inspector/domains.py` 加一個 `DomainProfile`（權重路徑、類別表、prompt、報告詞彙、已知侷限），不用碰 `detector.py`/`pipeline.py`/`report.py`。**VisDrone 資料集僅限學術研究用途**，本 repo 不隨附任何無人機影像，需自備測試圖。UAV 權重同樣不隨 repo 發佈，需另外下載到 `weights/`：
